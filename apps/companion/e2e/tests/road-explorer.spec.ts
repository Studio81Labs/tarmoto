...
test.describe("road quality explorer", () => {
  ...
  test("T27/T28: public explorer exposes regional closures and passes panels", async ({
    ...
    try {
      ...
      // Closures + Passes are opt-in info layers (#570); the map-points
      // unification collapsed their two pills into one "Conditions" toggle
      // that opens both docked panels together. Toggle it on before asserting.
      await page.getByRole("button", { name: /^conditions\s*$/i }).click();
