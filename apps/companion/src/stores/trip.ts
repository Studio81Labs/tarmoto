...
interface TripState {
   ...
   * reverse-geocoded pin name re-arms Save without staling previews or forcing
   * a re-route. Cleared on save/load alongside `routeDirty`.
   */
  namesDirty: boolean;
  ...
  (set, get) => ({
    ...
    renameWaypoint: (waypointId, name) =>
      set((state) => {
        ...
        // re-arms without staling previews or forcing a re-route.
