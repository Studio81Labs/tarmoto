...
test("non-/admin paths fall through to the static ASSETS binding", async () => {
...
test("api requests return 503 when the proxy is unconfigured", async () => {
  const res = await handleAdminRequest(
    new Request("https://admin.tarmoto.test/admin/metrics"),
    { ASSETS: ENV.ASSETS },
  );
  assert.equal(res.status, 503);
});

test("proxies /admin/* to the backend, injecting the internal token and re-derived IP", async () => {
  let captured;
  const fetchImpl = (req) => {
    captured = req;
  ...
  assert.equal(upstream.pathname, "/admin/metrics");
...
test("preserves multiple Set-Cookie headers from the backend", async () => {
  ...
  const res = await handleAdminRequest(
    new Request("https://admin.tarmoto.test/admin/auth/login", {
      method: "POST",
    }),
    ENV,
    fetchImpl,
