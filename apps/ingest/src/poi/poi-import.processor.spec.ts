...
import { POI_IMPORT_JOB, type PoiImportRegion } from "@tarmoto/ingest";
import type {
  PoiImportResult,
  PoiImportService,
} from "./poi-import.service.js";
import type { PoiImportRunRecorder } from "./poi-import-run.recorder.js";
import type { PoiImportProducer } from "./poi-import.producer.js";
...
describe("PoiImportProcessor", () => {
  ...
  function build(importers: PoiImportService[]): {
    ...
    const producer = { enqueuePoiImportRegion } as unknown as PoiImportProducer;
    return {
  ...
  it("dispatch: skips (no fan-out) when no source is enabled", async () => {
    ...
    await expect(
      processor.process(jobNamed(POI_IMPORT_JOB.DISPATCH)),
  ...
  it("dispatch: fans out one job per (source, region) across enabled sources with a global stagger", async () => {
    ...
    const result = await processor.process(jobNamed(POI_IMPORT_JOB.DISPATCH));
  ...
  it("import-region: routes the job to its source and runs importRegion", async () => {
    ...
    const result = await processor.process(
      jobNamed(POI_IMPORT_JOB.REGION, { code: "SK", source: "fsq" }),
    );
  ...
  it("import-region: defaults a missing source to osm (a job enqueued before the source field existed)", async () => {
    ...
    await processor.process(jobNamed(POI_IMPORT_JOB.REGION, { code: "SK" }));
  ...
  it("import-region: throws on a source not in the registry", async () => {
    ...
    await expect(
      processor.process(
        jobNamed(POI_IMPORT_JOB.REGION, { code: "SK", source: "xyz" }),
      ),
  ...
  it("import-region: throws on a code outside the source coverage list", async () => {
    ...
    await expect(
      processor.process(
        jobNamed(POI_IMPORT_JOB.REGION, { code: "ZZ", source: "osm" }),
      ),
  ...
  it("import-region: throws when the job is missing a code", async () => {
    ...
    await expect(
      processor.process(jobNamed(POI_IMPORT_JOB.REGION, {})),
