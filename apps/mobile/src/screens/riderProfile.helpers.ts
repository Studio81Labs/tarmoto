...
/**
 * "Following since 5 May 2026" — used on the followers/following list rows.
 * Uses British formatting (DD Mon YYYY) for compactness; matches the rest
 * of the mobile app's date rendering.
 */
export function formatFollowedSince(followedAt: string): string {
  const date = new Date(followedAt);
  if (Number.isNaN(date.getTime())) return "Following";
  return `Following since ${date.toLocaleDateString("en-GB", {
    day: "numeric",
    month: "short",
    year: "numeric",
  })}`;
