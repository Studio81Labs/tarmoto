...
describe("Stamp", () => {
  it("renders its label", async () => {
    await render(<Stamp>Roads near you</Stamp>);
...
describe("Metric", () => {
  it("renders value, unit and label", async () => {
    await render(<Metric value="186" unit="km" label="This week" />);
...
describe("BrandButton", () => {
  it("renders its label and fires onPress", async () => {
    ...
    await fireEvent.press(screen.getByText("Start commute"));
    expect(onPress).toHaveBeenCalledTimes(1);
  });
  ...
  it("does not fire onPress when disabled", async () => {
    ...
    await fireEvent.press(screen.getByText("Generate loop"));
...
describe("QualityBars", () => {
  ...
  function barColors(tree: unknown): string[] {
    const out: string[] = [];
  ...
  it("fills exactly q bars with the score's ramp colour", async () => {
    const { toJSON } = await render(<QualityBars q={3} empty="#000000" />);
  ...
  it("rounds a fractional score so colour and fill count agree", async () => {
    // 3.6 rounds to bucket Q4: four bars filled, all in the Q4 colour.
    const { toJSON } = await render(<QualityBars q={3.6} empty="#000000" />);
    const colors = barColors(toJSON());
    expect(colors.filter((c) => c === QUALITY_COLORS[3])).toHaveLength(4);
    expect(colors.filter((c) => c === "#000000")).toHaveLength(1);
  });

  it("defaults empty bars to the dark palette tone on dark surfaces", async () => {
    const { toJSON } = await render(<QualityBars q={2} onDark />);
    const colors = barColors(toJSON());
    expect(colors).toContain(brandColorsDark.qEmpty);
    expect(colors).not.toContain(brandColorsLight.qEmpty);
  });

  it("exposes the score to screen readers, with an override", async () => {
    const { rerender } = await render(<QualityBars q={4} />);
    expect(screen.getByLabelText("Quality 4 of 5")).toBeTruthy();
    await rerender(
      <QualityBars q={4} accessibilityLabel="Stelvio surface: great" />,
    );
    expect(screen.getByLabelText("Stelvio surface: great")).toBeTruthy();
  });

  it("renders an unscored placeholder for null/undefined, not Q1", async () => {
    // Unmeasured road: no filled bars, announced as unscored.
    const { toJSON } = await render(<QualityBars q={null} empty="#000000" />);
    const colors = barColors(toJSON());
...
describe("Toggle", () => {
  it("exposes switch role and checked state, and fires onToggle", async () => {
  ...
  it("does not fire when disabled", async () => {
    const onToggle = jest.fn();
    await render(
      <Toggle on onToggle={onToggle} disabled accessibilityLabel="x" />,
    );
  ...
  it("expands the touch target to the glove-first 44px minimum", async () => {
    ...
    await render(<Toggle on={false} accessibilityLabel="x" />);
...
describe("icon colour injection", () => {
  function strokeColors(tree: unknown): string[] {
    const colors: string[] = [];
    const visit = (node: unknown): void => {
      if (!node || typeof node !== "object") return;
      const element = node as {
        children?: unknown[];
        props?: { stroke?: unknown };
      };
      if (typeof element.props?.stroke === "string") {
        colors.push(element.props.stroke);
      }
  ...
  it("BrandButton injects its label colour into a bare icon", async () => {
    // Default ink button -> cream label/icon colour, not black.
    const { toJSON } = await render(
      <BrandButton icon={<BrandIcon name="nav" />}>Start</BrandButton>,
    );
    expect(strokeColors(toJSON())).toContain("#F5EFE6");
  });

  it("does not override an explicit icon colour", async () => {
    const { toJSON } = await render(
      <BrandButton accent icon={<BrandIcon name="nav" color="#123456" />}>
        Go
      </BrandButton>,
    );
    expect(strokeColors(toJSON())).toContain("#123456");
  });
});
