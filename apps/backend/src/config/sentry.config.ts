 ...
 * `null` and `instrument.ts` skips `Sentry.init`, so the backend runs exactly
 * as before. Once the DSN is provided (e.g. in Coolify) it activates with no
 * further code change. The release defaults to the version the deploy stamps
 * into `TARMOTO_SENTRY_RELEASE` / `TARMOTO_APP_VERSION`
