 ...
 * This is exactly the artifact the (follow-up) osmium tag-injection pass
 * consumes: for each way id, the single `smoothness` value to write onto it.
 */
 ...
 *
 * This service is the **engine-neutral core**: it produces the per-way
 * `smoothness` assignments by joining our ~100 m segments back to their OSM way
 * via `osm_way_id` and picking a representative value per way. Writing those
 * tags into a derived `.pbf` (osmium) and triggering a GraphHopper graph
 * re-import is the deployment-shaped follow-up documented in the module README —
 * this piece is pure, unit-tested query + mapping with no infra dependency.
 *
