...
| `Info.plist` `UIApplicationSceneManifest`            | ✅ Declares the `CPTemplateApplicationSceneSessionRoleApplication` scene pointing at `$(PRODUCT_MODULE_NAME).CarSceneDelegate` (the Swift class shipped in `apps/mobile/ios/TarmotoApp/CarSceneDelegate.swift`)                                                                                                                                        |
| `CarSceneDelegate.swift`                             | ✅ App-level `CPTemplateApplicationSceneDelegate` that forwards `didConnect` / `didDisconnect` into `RNCarPlay.connectWithInterfaceController:window:` and `RNCarPlay.disconnect`. Required because the package does not ship a ready-made scene delegate (verified against `node_modules/react-native-carplay@2.4.1-beta.0/ios/RNCarPlay.{h,m}`).     |
...
| Xcode project (`project.pbxproj`)                    | ✅ `CODE_SIGN_ENTITLEMENTS = TarmotoApp/TarmotoApp.entitlements` set on Debug + Release; `TarmotoApp.entitlements` and `CarSceneDelegate.swift` added as PBXFileReferences and the Swift file added to the target's PBXSourcesBuildPhase.                                                                                                              |
...
| Area                                                                                   | State after #498                                                                                                                                                                |
| -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `androidx.car.app:app` Gradle dependency                                               | ✅ Pinned to stable 1.7.0 in `apps/mobile/android/app/build.gradle`                                                                                                             |
