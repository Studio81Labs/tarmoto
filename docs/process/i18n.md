...
- Raw user-facing text on `label` / `title` / `alt` / `placeholder` /
  `aria-label` / `ariaLabel` JSX props is blocked by ESLint
  ...
  `// eslint-disable-next-line no-restricted-syntax -- <reason>`. The guard
  covers plain literals, template literals with static text, and conditional
  branches at any nesting — but it CANNOT see a raw label map rendered as JSX
  children (`{ROLE[type]}`), so type such maps
  `satisfies Record<K, EnglishMessageKey>` to keep `t(ROLE[type])` enforced
  (the residual gap is tracked in #1047).
