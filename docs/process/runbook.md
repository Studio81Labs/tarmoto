...
After any PaaS upgrade, app re-creation, or restore from backup, walk through this checklist before merging the next PR to `main`:

1. PaaS UI → Production application → **Advanced Settings → Deployment**.
...
4. Push a no-op commit to a feature branch, merge to `main`, watch GitHub Actions:
   ...
   - The "Trigger production deploy (production only)" step is **skipped**.
   - "Wait for deploy to go live" + "Smoke test" both target `BACKEND_URL_STAGING`.
5. Production application's deploy history shows **no** new deploy from this main push.
6. Tag the same commit, push the tag, confirm production deploys and the smoke runs against `api.tarmoto.app`.

If step 5 fails (production redeployed on a main push), the Auto Deploy toggle has flipped back ON — re-disable it and audit why (PaaS upgrade reset config, manual change, etc.).

### Rollback (production)

The CI auto-rolls back on smoke failure. To force a manual rollback:

1. PaaS UI → Production application → **Deployments**.
...
- `COOLIFY_API_TOKEN` — for poll + rollback (issued from the PaaS dashboard)
