...
> - **12 of 14 `sys_*` system switches enforced** (subsystems degrade on
>   `force_off`); 2 pending — see §6.2 Phase 2b.
>
> **Launch posture is nuanced** — NOT a blanket "everyone unlimited":
>
> - A **launch-mode global override is seeded on 10 keys**: `gpx_export`,
...
>   `max_trip_collaborators`, `road_quality_max_zoom` (`null`, 1818) +
>   `max_group_ride_members` (`null`, 1819). Those 10 resolve unlimited for every
>   rider regardless of tier (barring a stricter per-user override), so **all 9
>   enforced entitlements are dark today**. **Go-live must clear all 10
>   overrides**, not just the limits. (`road_quality_full_zoom` is the 10th —
>   seeded but not itself enforced; the zoom capability is gated by the
>   `road_quality_max_zoom` limit.)
...
- **System switches (§3) — mechanism + admin SHIPPED** (Phase 2). A **third registry kind** (`kind: "system"`, `default: true`, no tiers) with all 14 `sys_*` keys, resolved by the pure `resolveSystemSwitch`/`buildSystemSwitchSnapshot` (on unless an operator `force_off`) and `FeatureResolver.getSystemSwitches`. Overrides reuse the existing global `feature_states` table and already ride on `GET /config/flags`; a dedicated `/admin/system-switches` surface (disable/enable, reason-required) manages them, grouped separately in the admin console, and they never ride on the per-user `/users/me` payload. **Enforcement has since shipped for 12 of the 14** (see §6.2 Phase 2b) — most subsystems consult `FeatureResolver.isSystemSwitchEnabled` (service-level; mostly silent, but some paths raise 503/404) or `@RequireSystemSwitch` (`SystemSwitchGuard`, **503**) when a switch is `force_off`. Two switches gate purely **client-side** subsystems the backend can't stop (the phone's raw accelerometer sampling, the companion's directly-fetched aerial raster), so those clients read the same `GET /config/flags` map and enforce locally. Degradation is per-endpoint, not uniform per switch (see §3).
...
**Phase 2b — system-switch enforcement (12 of 14 shipped).** These `sys_*` switches now actually stop / degrade their subsystem when an operator flips them `force_off`:
...
**Still pending (2):** `sys_surface_ml_classification`, `sys_booking_affiliate` — registry + admin toggle exist, but no feature yet consults them (the surface-ML on-device classifier and the booking-affiliate integration aren't built).
...
- **Enforcement for the rest of §1/§2.** The 9 above gate. Paid features with NO gate yet: `offline_maps` / `max_offline_regions` (mobile-client-only — SP2), `road_quality_full_zoom` (the boolean; the paired `road_quality_max_zoom` limit _is_ enforced, so the zoom capability is gated — the toggle is upsell-copy only), and the not-built `priority_hazard_alerts`, `advanced_analytics`, `api_access`, `garmin_export` (deferred — gating ships with the feature).
...
- **Per-switch enforcement:** 12 of 14 `sys_*` switches are enforced (see §6.2 Phase 2b); the remaining 2 (`sys_surface_ml_classification`, `sys_booking_affiliate`) are registry + admin only — their subsystems aren't built yet.
