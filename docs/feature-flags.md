...
- **SP4 — Free-tier kill switches (mobile).** The 11 §1.1 free flags are "free for everyone" toggles that exist only so an operator can globally KILL a misbehaving free feature. They are enforced fail-SAFE (default ON; only an operator `force_off` disables) off the public `/config/flags` fast path — NOT the fail-closed per-user snapshot, because the app has no login gate so a signed-out rider (whose snapshot is absent) reaches these features. Shared `resolveFeatureKillSwitch` + mobile `isFeatureKillSwitchActive` (over the existing `systemSwitchCache`, refreshed cold-start + foreground; render-time gates use the reactive `useFeatureKillSwitchActive` so a persisted `force_off` tears the surface down promptly). **PR1 (5, safety-critical) shipped:** `crash_detection` (detector never subscribes + belt-and-braces block on the SOS POST), `ride_tracking` (no recording; bounce back), `hazard_alerts` (map REST+WS reception, markers, toggle, CarPlay hazard mirror, and the app-level commute alert incl. an in-flight recheck), `basic_navigation` (map-only fallback — no GPS session/maneuvers/voice, and stops the head-unit turn-by-turn projection), `weather_alerts` (no route-weather fetch/banner). **`carplay_android_auto` deferred to a follow-up:** a correct whole-projection kill needs a bridge-level "inert root + block re-mount" teardown — the current CarPlay/AndroidAuto unmount primitives only swap Tarmoto surfaces (iOS `unmountQuickActions` is a no-op; nav `stop` restores the ride board), so gating the bands would leave interactive surfaces on the head unit. **PR2 (5, post-auth) pending:** `road_quality_overlay`, `hazard_reporting`, `trip_planning`, `gpx_import`, `community_access`.
