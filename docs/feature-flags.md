...
> - **13 of 14 `sys_*` system switches enforced** (subsystems degrade on
...
**Phase 2b — system-switch enforcement (13 of 14 shipped).** These `sys_*` switches now actually stop / degrade their subsystem when an operator flips them `force_off`:
...
- **Enforcement for the rest of §1/§2.** The 9 above gate. Paid features with NO gate yet: `offline_maps` / `max_offline_regions` (mobile-client-only — SP2), and the not-built `priority_hazard_alerts`, `advanced_analytics`, `api_access`, `garmin_export` (deferred — gating ships with the feature). (`road_quality_full_zoom` was retired — the `road_quality_max_zoom` limit is the sole zoom-depth gate.)
...
- **Per-switch enforcement:** 13 of 14 `sys_*` switches are enforced (see §6.2 Phase 2b); the remaining 1 (`sys_booking_affiliate`) is registry + admin only — its subsystem isn't built yet.
