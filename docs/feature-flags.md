...
- **System switches (§3)** are modelled as a **third registry kind** (`kind: "system"`, `default: true`, no tiers), resolving through the existing global `feature_states` table (operator `force_off` kills a switch for everyone) and served on `/config/flags`. They reuse the entitlement machinery but are grouped separately in the admin console and never ride on the per-user `/users/me` payload.
