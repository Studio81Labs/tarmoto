...
| `pois` **PostGIS store** + weekly BullMQ importer (#745) | **Built but dormant** (`TARMOTO_OSM_POI_IMPORT_ENABLED=false`); **table is write-only, nothing reads it** | _(as of the POI-ingestion extraction, T1–T7)_ `packages/poi-db/src/entities/poi.entity.ts`, `packages/poi-db/src/migrations/1787000000000-AddPois.ts`, `apps/ingest/src/poi/poi-import.service.ts`, `modules/jobs/*` (queue `poi.import`, `WEEKLY_SUN_0300`) |
...
- **Coverage list (config-driven):** CZ, SK, PL, DE, AT, IT, SI, HR, BA, RS, ME, MK, AL, XK, BG, RO, GR. `TARMOTO_OSM_POI_IMPORT_REGIONS` replaces the single `TARMOTO_OSM_POI_IMPORT_BBOX`.
...
1. **Match** OSM POIs to a commercial place (name-normalized + geo-proximity within ~50 m + category agreement), storing only `fsq_id` / `google_place_id` + `enrichment_matched_at`. **In v1 (§6.1) matching is lazy** — only for the POIs actually being enriched — and its provider calls count against the **same request-count budget lock** (they are part of the "~1–2 calls/POI"). An **import-wide bulk matcher** (a low-priority pass after each Geofabrik run) would issue a match request per imported POI outside any nav/detail trigger and the lock, so it is **deferred to a later paid phase** where the free-tier quota isn't the constraint.
...
- **Mobile-nav gated (v1).** The **nav-start batch** is mobile-only — only active navigators trigger it; companion route-planning does not batch-enrich in v1 (a cost decision, revisitable). The separate §6 detail-view trigger is unaffected (available on both clients, same budget lock), so "mobile-only" scopes the batch, not all enrichment. The batch _trigger_ is client-gated (a mobile-nav flag / dedicated enriched endpoint); the _budget_ is server-enforced.
...
- **Phase 2 — Continent-scale ingestion.** Geofabrik + `osmium` bulk import (reuse roads pattern), `TARMOTO_OSM_POI_IMPORT_REGIONS`, staggered per-country weekly jobs, bbox-bounded stale tombstoning, index/volume tuning. _Scope: `backend`, `infra`, `docs`._
- **Phase 3 — Commercial enrichment.** `EnrichmentProvider` (FSQ primary / Google), OSM↔commercial matcher (store ids only), `GET /poi/:id` on-demand hydration + Redis cache + budget/rate guards, POI detail sheet (ratings/photos/reviews/price) with attribution. _Scope: `backend`, `companion`, `mobile`, `openapi`._ (The §6.1 nav-start enriched endpoint + response shape changes the API contract — regenerate the OpenAPI spec so clients pick it up.) **v1 lands per §6.1** (FSQ-only, mobile navigation-start batch, atomic budget lock, degrade-to-OSM). **Commercial attribution ships in this phase, with the surfaces it enriches** — both the detail sheet _and_ the nav list/map carry FSQ branding (or Phase 3 pre-warms the cache and defers nav display until Phase 4, per §6.1/§8's "no enriched field without branding"); it is **not** deferred to Phase 4. Google + companion enrichment are fast-follows.
- **Phase 4 — Compliance + shared contract.** ODbL "© OpenStreetMap contributors" wired into all POI surfaces, plus commercial attribution for any **post-v1** provider/surface (Google, companion enrichment) — v1's FSQ attribution ships with its display in Phase 3, not here; move POI enums/types into `@tarmoto/shared`; ADR + data-sources doc update. _Scope: `shared`, `companion`, `mobile`, `backend`, `docs`._
