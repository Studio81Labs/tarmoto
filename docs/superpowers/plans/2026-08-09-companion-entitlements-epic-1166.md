...
- [ ] Suppress the CTA **only when this rider's upgrade actually needs Checkout** — which is _not_ the same as `currentTier === "free"`. `paidPlanNeedsCheckout` (`page.tsx:277`) routes **every** plan action to Checkout for a rider on an operator-granted or cancelled paid tier, so a Pro rider in that state would otherwise get a Premium CTA leading to a page where nothing can proceed.
      **This needs a data seam, because `UpgradePrompt` has no snapshot and must not gain call-site props.** `paidPlanNeedsCheckout` derives from `snapshot.preview`, `currentPlan.tier` and `currentPlan.status === "canceled"` (`page.tsx:277-281`) — the `accountApi.getSubscription()` payload, not `/users/me` — so neither the component's `currentTier` prop nor `useEntitlements` can answer it.
      Ship the seam in PR 1: a pure `upgradeNeedsCheckout(snapshot)` in `lib/subscription.ts`, plus a `useUpgradeRouting()` hook in `hooks/` that reads the snapshot through **react-query under a shared key** and applies it. Call sites stay untouched and all prompts share one cached request rather than one per prompt. The billing page currently fetches this in a bare `useEffect` (`page.tsx:184-194`), so PR 2 should consume the same hook instead of keeping a second copy of the state.
