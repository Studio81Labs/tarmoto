...
- [ ] Suppress the CTA **only when the upgrade actually needs Checkout** — i.e. when `currentTier === "free"`. Reuses the existing `target === null` no-CTA path, so `modalTitle` becomes "Limit reached" and `cta` is `null` on both variants with no new branch.
