...
- [ ] Suppress the CTA **only when this rider's upgrade actually needs Checkout** — which is _not_ the same as `currentTier === "free"`. `paidPlanNeedsCheckout` (`page.tsx:277`) routes **every** plan action to Checkout for a rider on an operator-granted or cancelled paid tier, so a Pro rider in that state would otherwise get a Premium CTA leading to a page where nothing can proceed.
      **This needs a data seam, because `UpgradePrompt` has no snapshot and must not gain call-site props.** `paidPlanNeedsCheckout` derives from `snapshot.preview`, `currentPlan.tier` and `currentPlan.status === "canceled"` (`page.tsx:277-281`) — the `accountApi.getSubscription()` payload, not `/users/me` — so neither the component's `currentTier` prop nor `useEntitlements` can answer it.
      Ship the seam in PR 1: a pure `upgradeNeedsCheckout(snapshot)` in `lib/subscription.ts`, plus a `useUpgradeRouting()` hook in `hooks/` that reads the snapshot through react-query and applies it.
      **Key it on `userId`, the way `USERS_ME_QUERY_KEY(userId)` already is.** A bare shared key leaks across an account switch in one browser session: the client's `staleTime` is 30s (`AuthenticatedAppProviders.tsx:25`) and `AuthSync` clears only the Zustand session (`AuthSync.tsx:30`), never React Query — so the second rider can be classified from the first rider's snapshot and get either a suppressed portal CTA or a dead Checkout one. Add an account-switch test.
      **The hook must answer free and anonymous riders without any request.** `GET /account/subscription` is `AuthGuard`-protected (`account.controller.ts:45-47`), and `UpgradePrompt` renders on the **public** `/explore` for logged-out visitors with a synthetic `currentTier={tier ?? "free"}` (`QualityMap.tsx:1362-1373`) — so an unconditional fetch would 401 on every anonymous zoom prompt. It is also unnecessary: a free tier _always_ routes to Checkout, so only a **paid** rider needs the snapshot to tell portal from Checkout. Enable the query on **`authenticated`** alone — only an **anonymous** visitor can safely skip it. An authenticated _free_ rider cannot: `managed_by` comes from `user.subscription_provider` (`account.service.ts:2156`), independent of tier, and the billing page keys the store panel purely on `snapshot.managedBy` (`page.tsx:288`). So a lapsed App Store or Play Store rider reads `currentTier: "free"` while their management path is the **store**, which `sys_billing_checkout` does not gate at all — short-circuiting them to "needs Checkout" would remove a CTA that leads somewhere still working. Anonymous short-circuits to "needs Checkout"; everyone else waits for the snapshot. Tests: anonymous prompt (no request), and a store-managed free rider (CTA retained).
      **Unresolved must mean "keep the CTA", never "suppress".** While that query is loading or has failed, the hook cannot tell an active paid subscriber (portal — CTA must stay) from a cancelled or granted one (Checkout — CTA goes). Defaulting the unknown to "needs Checkout" would strand an active paid rider with no upgrade route on every slow load, which is the trapping failure PR 2 exists to prevent. Suppress **only** once the snapshot confirms Checkout routing. This matches the fail-safe direction already required of `useSystemSwitch`, and needs its own tests: active paid rider with the snapshot loading, and with it errored. Call sites stay untouched and all prompts share one cached request rather than one per prompt. The billing page currently fetches this in a bare `useEffect` (`page.tsx:184-194`), so PR 2 should consume the same hook instead of keeping a second copy of the state.
      This adds files to PR 1 (`hooks/useUpgradeRouting.ts` + test, `lib/subscription.ts`) — still small, but no longer the three-line change §0.4 described. Three findings in this review came from "is this rider's route Checkout or portal" being answered inconsistently in different places, so one definition is worth the seam. Reuses the existing `target === null` no-CTA path, so `modalTitle` becomes "Limit reached" and `cta` is `null` on both variants with no new branch.
...
- [ ] **Gate the community quality cluster — authenticated, and currently ungated.** `road_quality_overlay` is a global operator kill, so it must hide quality for signed-in riders too, not only anonymous ones. Verified ungated: `components/community/CommunityRideCard.tsx:27` (`scoreToQualityTier(ride.avg_road_quality)` — the file imports `useFeatureKillSwitch` but only for `trip_planning` at `:50`), `components/community/SharedRidesSection.tsx:160` (same call on rider profiles), `community/collections/[collectionId]/page.tsx:839-842` (`QualityBars`), the discover collection detail via `CollectionRouteRow`, and **both** of the feed's quality controls — the `highest_quality` sort (`community/feed/page.tsx:45`) and the **minimum-quality filter** (`:61`, `:84`, `:198`), which `lib/community-feed.ts:42-43` turns into a `min_quality` query parameter. Gate the rendering **and** every quality-dependent control: a sort or filter the operator killed should not stay selectable, and must stop being sent.
...
- [ ] Same cluster, three more paths — all verified ungated: `rides/_components/RideKpiCards.tsx:58-64,94` renders `stats.avg_quality`; `rides/_components/RidesFilters.tsx:213-239` keeps issuing `min_quality` / `max_quality` **queries**, so the killed dimension is still being asked for even when nothing renders it; and `/rides/stats` renders the client-computed `QualityTrendCard` (`page.tsx:185-190,401`). The last still matters after PR 11 — that gate locks the route for non-entitled riders, but a **Premium** rider would keep seeing a quality trend the operator killed.
- [ ] `components/TripImportDialog.tsx` — gates only `gpx_import` (`:40`) while its ready state renders `firstDay.avgQuality` (`:286-287`), a "Segment quality" list (`:299`) and each segment's tier (`:303`, `:326`). A rider can import a GPX and keep reading quality all through the kill. Test the initial killed state **and** a live flip with the preview already open.
- [ ] Verified **already handled**, do not re-fix: the road-map coverage drawer. `mapView` is derived (`road-map/page.tsx:128`), the sidebar prop is `mapView === "coverage" ? selectedSegmentId : null` (`:547-548`), an effect nulls the selection on `mapView` change (`:153-155`), and the fetch effect resets to `idle` and aborts in flight (`:280-289`). Recorded so the sweep does not keep re-opening it.
- [ ] **Widen the sweep definition accordingly.** "Every non-client `page.tsx`" was the wrong frame: it answers _what leaks into the HTML_ but not _what a visitor can still see_, and a kill switch is about both. The sweep is over **every surface rendering flag-gated data** — public or authenticated, client or server — server ones need the data stripped (HTML, `<head>`, JSON-LD, Flight payload); client ones need the affordance, the quality-_dependent_ controls and the quality-_only_ fetches gated. **Not the base fetch.** `/rides`, the community feed and the GPX import must keep loading their own rows — suppressing those takes unrelated functionality down and contradicts the operator rule below. Only quality-specific requests (Fun Zones' `/roads/fun-zones`, a quality-only detail call) stop; mixed responses keep arriving and simply render no quality.
...
**Scope:** `community_access` only. The same four routes also carry `road_quality_overlay` quality — that work is **PR 3a's** (files listed there); PR 4 depends on 3a, so 3a strips the quality and PR 4 then adds the availability gate.
...
- [ ] **The two flags are independent — but the `road_quality_overlay` half belongs to PR 3a, not here.** Keeping it in PR 4 would mix two flags into one deliverable, which is exactly what the per-flag sweep rule warns against; PR 4 stays `community_access`-only, and PR 4 already depends on 3a so the same files are touched in a defined order. **Listed here for context — implement in 3a:** with `road_quality_overlay` at `force_off` and `community_access` still on, killed quality data stays public on:
...
**Validation:** `pnpm --filter @tarmoto/backend test` as well as the companion suite; `pnpm openapi:gen` if the response shape moves. **If the shared-endpoint option is taken, add the mobile package's own `test`, `typecheck` and `lint`** — otherwise the shared review-read contract lands with an unvalidated third consumer, which is the whole reason this PR is `feat(cross)`.
...
- [ ] **The own row is for deletion only — it must not feed any aggregate.** `RoadReviewsPanel:216` pushes `reviews.length` through `onCountChange`, and `SegmentDetailSidebar:447` binds that straight to `setReviewCount` — so a one-element fallback array renders "1 review" as the **community** total, overwriting the neutralised `review_count` the backend deliberately zeroes. The panel's header average derives from the same array. Keep every count and average in the neutral/unavailable state while the switch is off, and surface the own row purely as "your review" plus its delete action. Cover the authenticated-owner case explicitly; a signed-out or non-owner test cannot see this.
...
- [ ] **Carry the trial state across Checkout — but never assert it from a URL parameter.** Stripe is a full cross-origin navigation: the page unmounts and remounts on `?checkout=success`, so a value held in React state before `window.location.assign` is gone by the time the banner renders. A plain `?trial=1` marker does **not** solve this: once it round-trips through the browser it is rider-forgeable, so anyone opening `?checkout=success&trial=1` gets a "your free trial has started" confirmation for a Checkout they never completed, while the poll never activates anything. Server-_generated_ is not server-_authoritative_ after a user-controlled URL.
      ...
      **Specify the failure state, not just the happy path.** Verification can fail three ways — an invalid session id, a valid one belonging to **another** rider (the binding check below rejecting it), and Stripe or the endpoint being unavailable. With only "Checking your plan…" permitted before verification, and a bounded poll that can finish while the account is still Free, an implementation is free to leave that banner spinning forever. Terminal state must be **neutral and dismissible**, making no checkout, payment or trial claim — the rider's plan grid is the source of truth and it is right there. Tests: invalid id, cross-user id, transient error.
...
- [ ] The copy map needs **two** dimensions, not one. `platform: "web" | "mobile" | "all"` annotates capabilities the companion does not implement (`offline_maps`, `group_rides`, …) rather than implying they work on web. And a **`status`** dimension decides whether a key is marketable at all.
- [ ] **`status: "unbuilt"` keys must never render — otherwise this PR makes the page worse, not better.** `api_access` and `garmin_export` are `PREMIUM_ONLY` grants in the registry (`feature-flags.ts:191-201`) but `docs/feature-flags.md:254` lists them among the **not-built** deferred features, and `PLAN_COPY` mentions neither today. Naive registry derivation would _newly_ advertise "Personal API token" and "Direct route export to Garmin" on a €49.99/yr card for functionality that does not exist — inventing two false claims while fixing four. `priority_hazard_alerts` is the same category and **is** on the card today, so this dimension also removes an existing false claim.
- [ ] Test that no registry key marked unbuilt appears on any plan card — the natural failure here is a new key being added to the registry, marked marketable by default, and silently advertised
...
- [ ] Non-entitled → keep `StatsPageHeader` and render a `LockedFeatureCard` teaser, so the route never shows an unexplained gap.
- [ ] **`LockedFeatureCard` must take the capability as a prop — today it hardcodes one.** `LockedFeatureCard.tsx:47` passes `capability={{ feature: "advanced_ride_stats" }}`, which a **Pro** rider already holds, so `upgradeTierForFeature` resolves no target and the card renders "Limit reached" with no Premium CTA — the exact upsell this workstream exists to create, silently absent for the tier most likely to buy. Parameterise the capability (defaulting to today's value so existing call sites are untouched) and cover the component and its tests in PR 11.
...
| **`road_quality_overlay`**  | **Feature flags** | _RSC (raw source):_ best-roads list + JSON-LD, and all three share routes carrying quality. _Client (interact + network tab):_ `/explore` Fun Zones; the community cluster (ride cards, rider profiles, collection detail, `highest_quality` sort **and** the min-quality filter); the personal ride cluster (ride list + quality sort, KPI cards, the `min_quality`/`max_quality` filters, ride detail, compare, `/rides/stats` quality trend, dashboard recents); the GPX import preview (`TripImportDialog` — import a file and check the avg-quality tile and segment list, then live-flip with the preview open) |
