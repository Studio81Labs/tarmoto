...
async findZoneById(zoneId: string): Promise<FunZoneDetailDto> {
  ...
  // against fun_zone_roads for an id that doesn't exist. Sequential
  // execution is also required by the `toHaveBeenCalledTimes(1)`
  // assertion in the 404 test above.
...
Visit `http://localhost:3002/explore`. Verify:
...
3. Visit `http://localhost:3002/discover` as an anonymous user (no login).
...
4. Regression: visit `http://localhost:3002/explore`.
