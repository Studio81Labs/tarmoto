...
- `buildShareSummary`/`lib/exploration.ts` — dead code candidate; needs an explicit deletion decision (kept as-is, untouched, through this migration rather than deleted opportunistically).
- Metric-pinned surfaces left deliberately raw: `RecentRidesTable`'s KM column (`Math.round(r.distance_km)` under a static "KM" header), the dashboard `KpiTileRow`'s "This month" tile (`format.integer(stats.this_month_km)` under `unit="KM"`), and the stats heatmap's day-cell title/tooltip (`` `${format.decimal(cell.distanceKm, 0)} km` ``). None of these convert the value with `splitDistanceKm`/`distanceKm` — the label is accurate for metric riders but the surface doesn't honor the imperial preference at all (a bigger behavior change than relabeling, so deferred rather than folded into the final-review unit-label fixes).
- `percent(fraction, digits?)` extension: `Formatters.percent()` is hardcoded to `maximumFractionDigits: 0`, so the 3 sites in `rides/stats/page.tsx` needing one decimal of precision (the twistiness breakdown rows, `` `${format.decimal(s.pct, 1)}%` `` at the bar label, its title attribute, and the legend row) fall back to manual `decimal(...) + "%"` instead of the seam. Add an optional `digits` param and migrate those 3 sites.
- Raw `t("{count} ...")` interpolation sweep: ~25+ call sites (achievements, community, trips, closures, passes, shared-link pages) interpolate a raw JS number into a `{count}` translation placeholder with no `format.integer()` pass, so large counts render ungrouped (e.g. "1234 views" instead of "1,234 views"). Not touched in this migration — needs its own pass since it's translation-catalog surface, not a formatter call site.
- Test debt: `MapPointPopover`'s closure detour line (`format.distanceKm(detourKm)`, already migrated) has no test assertion covering its rendered output; the achievements page test doesn't assert on badge-date formatting or the percent-to-next-tier computation post-migration.
- `formatNumber(value, digits=0, format)` (`lib/ride-detail.ts`) has a dead default: `digits`'s `= 0` default sits before the required `format` param, so no caller can actually omit `digits` positionally (TypeScript requires all params up to the last one you pass) — reorder to `formatNumber(value, format, digits = 0)` or make `digits` required.
