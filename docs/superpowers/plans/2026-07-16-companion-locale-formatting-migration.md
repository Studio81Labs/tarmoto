...
- ~~Metric-pinned surfaces left deliberately raw~~ — CONVERTED after Codex review flagged two of them: `RecentRidesTable`'s KM column, the dashboard `KpiTileRow` "This month" tile, the stats heatmap day tooltip, plus the home `TripDraftCard` KM slot and `formatCount`'s compact decimal (was `toFixed`-period regardless of locale; omitted-locale keeps the legacy form for mobile). The whole dashboard now honors the unit preference.
