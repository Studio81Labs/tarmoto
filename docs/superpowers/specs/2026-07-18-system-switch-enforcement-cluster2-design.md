...
- **Writes → clean 503** (`ServiceUnavailableException('Reviews are temporarily unavailable')`): `create` (`:249`), `update` (`:328`), `castVote` (`:589`), `uploadPhotos` (`:443`). **`delete` (`:385`) and `clearVote` stay allowed** — both are withdrawals, and per decision 2 a kill must never trap user content. `castVote` (the additive write) is gated; `clearVote` (retracting your own vote) is not, so a rider can still remove a vote during an incident instead of leaving it stuck in the aggregates.
  - **But `clearVote`'s response aggregate degrades to neutral when off.** The withdrawal (the `voteRepo.delete`) always runs, but the returned `helpful_count`/`not_helpful_count` are display data — and because `delete` is idempotent, an ungated read-back would let any caller use the DELETE as a read endpoint for the hidden vote counts. So when the switch is off, `clearVote` skips the aggregate query and returns neutral counts (`{ 0, 0, null }`), matching the zeroed `listForSegment`/road-detail reads. The switch IS consulted here — not to block the write, but to neutralize the read.
...
- **Writes:** `create`/`castVote`/`uploadPhotos` off ⇒ 503; `delete` off ⇒ allowed (switch not consulted); `clearVote` off ⇒ vote still deleted but the response aggregate is neutral (`{ 0, 0, null }`, switch consulted for the read only). `uploadPhotos` off ⇒ rejected in the guard phase before Multer parses the payload.
