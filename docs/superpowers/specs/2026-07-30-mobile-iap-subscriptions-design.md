...
- `POST /account/subscription/iap/apple/notifications` — ASSN v2. No user auth; verified by JWS signature. Find the rider by `apple_original_transaction_id` → sync tier/status/period_end for the full lifecycle: `DID_RENEW`, `DID_FAIL_TO_RENEW` (→ `past_due`; **tier held ONLY while the store is in a real billing-grace period** — the verified payload's grace/expiry dates drive this, not the event alone), `DID_RECOVER` (billing-retry success → `active`, since a recovery does NOT always emit `DID_RENEW`), `EXPIRED`, `GRACE_PERIOD_EXPIRED`, `DID_CHANGE_RENEWAL_STATUS`, `REFUND`, `REVOKE`. **Access is granted only while store entitlement is active or in grace; when grace is disabled/exhausted (retry without access) the tier drops to `free`, restored only on `DID_RECOVER`.** A terminal transition (`EXPIRED`/`REVOKE`/`REFUND`, no active sub left) drops the tier to `free` and **clears `subscription_provider`, the store id, and `plan_source`**.
- `POST /account/subscription/iap/google/notifications` — RTDN via authenticated Pub/Sub push. Find the rider by `google_purchase_token` → re-query the Play Developer API → sync the equivalent lifecycle: `SUBSCRIPTION_RENEWED`, `SUBSCRIPTION_IN_GRACE_PERIOD` (**hold — user still has access**), `SUBSCRIPTION_ON_HOLD` (**DROP access to `free` — account hold has no access; restore on `SUBSCRIPTION_RECOVERED`**), `SUBSCRIPTION_RECOVERED`, `SUBSCRIPTION_EXPIRED`/`SUBSCRIPTION_REVOKED`/`SUBSCRIPTION_CANCELED` (terminal clear). Also handle **`SUBSCRIPTION_PURCHASED`**: a deferred/pending purchase can complete while the app is closed — so persist enough account binding for a _pending_ token at `validate` time (bind the token to the rider even before it's `purchased`), and on `SUBSCRIPTION_PURCHASED` re-query Play and **atomically activate** the entitlement, so the rider isn't left free-after-charge until the client happens to revalidate. Access decisions are driven by the re-queried Play entitlement state, never the event name alone.
...
- **Provider-split cancellation.** Stripe subscriptions are server-cancelled today (unchanged). **Google CAN be server-cancelled** via the Play Developer API (`purchases.subscriptionsv2.cancel`), so deletion (and a duplicate/exclusivity conflict) must call it and reconcile failures — leaving future Play charges active because we treated Google like Apple would be a real billing bug. **Apple has no server-cancel API** — only Apple's manage-subscriptions surface can cancel — so App Store subscriptions keep the rider-must-cancel guidance.
