...
- `POST /account/subscription/iap/apple/notifications` — ASSN v2 (JWS-verified, no user auth). Lifecycle: `SUBSCRIBED` (resubscribe/reactivate → re-validate + re-claim), `DID_RENEW`, `DID_FAIL_TO_RENEW` (→ `past_due`; tier held ONLY during a real billing-grace window per the payload dates), `DID_RECOVER` (→ `active`; recovery need not emit `DID_RENEW`), `GRACE_PERIOD_EXPIRED` (grace exhausted → `free`), `DID_CHANGE_RENEWAL_STATUS` (auto-renew OFF while still paid → **keep tier + set `cancel_at_period_end`; do NOT terminal-clear** — access continues until the paid period actually ends), `EXPIRED` (period ended → terminal), `REFUND`/`REVOKE` (terminal).
...
- Purchase flow: buy → `POST /account/subscription/iap/validate` → refresh `/users/me` entitlements → finish/acknowledge the transaction. On backend failure, branch on the error kind rather than a blanket "don't finish": a **retryable** outage (5xx / network) leaves the transaction unfinished so the store re-delivers and a later validate succeeds. A **terminal** rejection (wrong `appAccountToken`, unknown product, ineligible-trial, exclusivity conflict) can never validate — but the two stores differ on how to close it out safely:
  - **iOS:** `finishTransaction` (Apple has no refund side-effect) and direct the rider to Apple's refund flow.
...
- Idempotent `validate` (transaction-id dedupe) + **durable notification dedupe**: a `processed_store_notifications` table with a UNIQUE key on the notification id (Apple `notificationUUID` / Google Pub/Sub message id) acting as a transactional inbox — the handler INSERTs the id in the same transaction as its side effects, and a duplicate delivery (Apple/Pub/Sub retries, a multi-pod deployment, or a restart with no shared in-memory record) hits the unique constraint and is skipped. Re-querying makes field updates idempotent on their own, but the one-shot side effects (emails, reconciliation records, cancellations, refunds) MUST be inbox-guarded so they can't run twice.
