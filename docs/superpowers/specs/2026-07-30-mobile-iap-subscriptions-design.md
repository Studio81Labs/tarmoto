...
- **Mobile unit:** `services/iap.ts` with `react-native-iap` mocked — buy→validate→refresh→finish; **separate assertions for the two failure classes**: a retryable outage leaves the transaction UNfinished (store re-delivers) while a terminal rejection FINISHES the transaction + surfaces refund/cancel guidance (so it can't re-surface the same error every launch); restore; exclusivity error surfacing; trial-vs-no-trial offer selection by eligibility. Paywall render + CTA wiring.
...
- **P0** — shared `SubscriptionProvider` + `IAP_PRODUCTS` + validate DTOs; `users` migration (provider + store id columns **with UNIQUE partial indexes**, backfill); the **atomic provider-claim** helper + applying it to the existing Stripe webhook/checkout **including clearing `subscription_provider`/`plan_source` on the Stripe terminal branch** (with tests); `GET /account/subscription` snapshot extended + **gated by provider** + trial-eligibility; **companion** normalization + store-managed "manage in store" panel + tests; the **preflight** store-management state for `DELETE /account` (confirm dialog / non-mutating first DELETE) + confirmation-UI/email warning.
- **P1** — Apple: `AppleBillingClient` (verify + derive tier from verified product + `appAccountToken` binding), `iap/validate` (Apple path), ASSN v2 webhook with the atomic claim + conflict reconciliation + grace-vs-hold access rules, Stripe/Apple exclusivity, trial-eligibility offer selection, terminal-vs-retryable validate result, tests.
- **P2** — Google: `GoogleBillingClient` (`obfuscatedExternalAccountId` binding + pending-purchase token binding), `iap/validate` (Google path), RTDN webhook with the atomic claim + `SUBSCRIPTION_ON_HOLD` access drop + `SUBSCRIPTION_PURCHASED` activation, same-provider second-token rejection, **server-side cancellation on deletion** (`purchases.subscriptionsv2.cancel`), exclusivity across all three, tests.
