...
- **Border-crossing ways are safe (no cross-region dedup needed).** The road segment identity is `(osm_way_id, segment_index)` with `segment_index` a 0-based ordinal within the way. With `complete_ways`, a way straddling the cz/sk border is emitted **complete and identical** in both extracts → both region runs produce identical `(osm_way_id, segment_index)` rows with identical geometry → the `ON CONFLICT` upsert is idempotent. Border ways get imported redundantly (once per adjacent region) but never collide or lose data — the same benign redundancy the POI folder model already has.
...
- **Empty/absent extract never wipes a region.** Absent file → skip; parsed-0-ways → skip. Only a genuine, non-empty extract drives reconciliation/tombstoning for its region.
