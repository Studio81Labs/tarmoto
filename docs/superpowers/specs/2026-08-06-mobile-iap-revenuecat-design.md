...
| 4.5  | **Provisioning spike — three required outputs: (f) identity across a plan upgrade, **(b)** correlation with two simultaneous subscriptions, and step 6.5's Google cancellation mechanism.** RevenueCat project, Play products with two plans, throwaway internal-testing build. Buy both plans: one upgrade observation answers (f), and the two live subscriptions answer (b) — the subscriber response is keyed by product id and carries no original transaction identifier, so there is nowhere else to check it cheaply. **Record all three before teardown.** Skip only if RevenueCat support answers **all three** first.                                                                                                                                                                                                                                                                                                                                                                                           | **next** — not started                         |
