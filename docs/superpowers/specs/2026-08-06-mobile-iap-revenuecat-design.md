...
| 4.9  | **Store subscription chains — the multi-chain storage move. Added 2026-08-12; BLOCKS step 5.** Design: `docs/superpowers/specs/2026-08-12-store-subscription-chains-design.md`. **Ships as TWO releases, not one.** Release A = expand (create `store_subscriptions` + indexes, vacuous backfill) + readers (`resolveEntitledTier` derives from the live set) + **store writers stop dual-writing the legacy Stripe-owned `users.subscription_*` fields — but STILL maintain `users.store_subscription_tier` and `store_subscription_tier_expires_at` atomically with the chain row.** "Chain-only" is the wrong reading: the same release makes every synchronous entitlement reader depend on that rollup, so a writer that skips it leaves store-only purchases null and every feature guard treats a paying rider as `free`. **Store claims must NOT dual-write `users.subscription_*`** — those columns become Stripe-owned at this step, and a `claimForGoogle` that still wrote them would overwrite a rider's Stripe Premium state with the Google chain's tier. There is nothing to dual-write anyway: every store column is NULL in every environment. Also updates `/users/me` to serve the billed tier across ALL sources (see the design's mobile-contract section) — without it the mobile activation poll never observes a store purchase. **And carries the entitlement readers with it:** `resolveEntitledTier` has 13 call sites and the enforcement path selects only `{id, subscription_tier, grant_tier}`, so chain-only writes without this leave every feature guard and admin limit reader resolving a paying rider as free — worse than not shipping, since the rider is shown Pro and then denied it. Rolled up into a maintained `users.store_subscription_tier` so the resolver stays synchronous and no feature check gains a query. Release B = contract (drop `apple_original_transaction_id` / `google_original_transaction_id` and their unique indexes), **gated on release A being fully deployed and no longer a rollback target** — dropping a column the `User` entity still maps is migration 1831's `42703` rolling-deploy hazard, and the vacuous backfill does NOT neutralise it because the hazard is the column name in TypeORM's select list, not the rows. Migration 1833's never-deployed shortcut is a statement about deployment state and must be re-checked then, not inherited. **This must precede the consumer**: building step 5 against the retired single-slot guard would produce a claim path that rejects a legitimate Play plan upgrade and then have to be rewritten. Also carries the deterministic response projection. **The multi-chain account-deletion enumeration is NOT in this step** — it calls `GET /v1/subscribers/{app_user_id}`, and no RevenueCat backend client exists until step 5, which is itself blocked on this step; scheduling it here makes release A uncompletable in its own order. 4.9 carries only the **schema prerequisites** for it (the purge-safe per-chain retry entries); the RevenueCat-backed enumeration and cancellation land with **step 6.5**, where deletion lives and the client exists. **Also re-scopes `uq_sbr_open_apple_otid_reason` (migration 1823) in the SAME release** — it keys one OPEN Apple reconciliation per `(otid, reason)`, which pairwise overlaps break: an Apple source overlapping both Stripe and Google promotes two rows sharing that OTID and reason, and the second fails `23505`, silently losing a real double-billing case on the escalation path.                                                                                                                                                                                                                            | **not started — blocks step 5**                                                                                                                |
