...
| (f)  | **✅ SUPERSEDED 2026-08-12 — do NOT build the equality-only guard.** This row previously read "gates ENABLING Play purchases; build with the equality-only guard; Apple-only enablement is not gated at all", and all three clauses are now wrong: the fork is decided (multi-chain storage), the single-slot binding is retired, and mobile IAP ships both stores together so there is no Apple-only enablement to be ungated. The consumer is built against the chain table — see step **4.9**, which must land before step 5. **UPDATED 2026-08-12: support has now answered, and the answer CONFIRMS the defect — a Play replacement does present a new `original_transaction_id`.** That does not change what this row gates (still Play enablement, still not building), but it removes the possibility that the equality guard was already correct. The replacement branch is now blocked on an **architectural fork, not on an observation**: RevenueCat exposes no `linkedPurchaseToken`, so no RC-visible field separates a store-confirmed supersession from a second independent subscription. See (f) for the fork and its collision with (b)/(h) and §1's no-migration call.                                                                                                                                                                                                |
