...
| Companion subscription page | **NO LONGER UNCHANGED (superseded 2026-08-12)** — the store panels do render from `managed_by`, but release A requires two companion changes: the Stripe **portal must be reachable whenever a Stripe side exists**, independently of `managed_by` (today `isStoreManaged` suppresses it), and **`payment_method` / `billing_history` must be labelled as Stripe** when the representative is a store chain, or a real card and real invoices are shown as though they describe the Apple/Google plan. Carried in step 4.9 — see the chain design's projection section |
...
**Consequence: no shared-contract change, and additive
column migrations only (plus a later drop — see the corrections below).** ~~no companion
change~~ — **that clause is superseded (2026-08-12)**: release A changes the companion's
portal gating and the attribution of Stripe payment method and invoices, per the row above.
The DTO shape is still unchanged, which is what the clause originally meant; the _rendering_
is not. This is what
makes the option cheap, and it is the reason to prefer mapping over introducing a
`revenuecat` provider value.

> **⚠️ THIRD CORRECTION (2026-08-12) — the SINGLE-SLOT STORE BINDING IS RETIRED.**
> Everything below about there being no Play purchase token stands, and so does
> the surrounding section's case for mapping RevenueCat onto the existing
> providers rather than adding a fourth. What does **not** stand is the shape the
> binding lives in: one nullable store id column per provider on the `users` row.
>
> RevenueCat support confirmed a Play plan change **rebases**
> `original_transaction_id` (open item (f)), and their recommendation — allow a
> rider more than one store transaction chain — is a storage instruction. The
> decision (2026-08-12) is to take it: store chains move to their own table, and
> entitlement derives from the live set. **Design:
> `docs/superpowers/specs/2026-08-12-store-subscription-chains-design.md`.**
>
> This reopens the "no migration" half of §1's key design call **for the store
> bindings only** — `SUBSCRIPTION_PROVIDERS` still stays `stripe|apple|google`,
> RevenueCat is still an ingestion channel and not a provider, and Stripe does not
> move. It also resolves open items **(f)**, **(h)** and the storage half of
> **(b)** together, because all three were this one assumption.
>
...
>
> **Step 4.75 is NOT complete.** This is the fence half only. Four items were
> listed here as remaining; **two of them were mis-scoped — corrected
> 2026-08-08.** The list now reads:
>
> | Item                                    | Status                   |
> | --------------------------------------- | ------------------------ |
> | `sbr_resolution_check` migration        | ✅ done — migration 1834 |
> | retirement on a successful Stripe claim | ✅ done                  |
> | rider row lock in the claim transaction | **step 5, not 4.75**     |
> | transaction-safe reconciliation dedup   | **step 5, not 4.75**     |
> | acquire-to-stamp gap                    | ✅ done — migration 1835 |
>
> **✅ STEP 4.75 IS COMPLETE (2026-08-08).** The acquire-to-stamp gap is closed by
> a **DB-backed lease**, and the harness adjudicated it exactly as this section
> demanded: the `it.failing` case now passes as `it`, and four mutations of the new
> guards each break a distinct test.
>
> **Why the lease and not the Redis `INCR` token.** "Token order equals acquisition
> order" only holds if ONE system decides both. Acquisition was Redis and
> allocation was PostgreSQL, so a stall between them reordered one against the
> other — the whole bug. Two ways to collapse that:
>
> - **Redis `INCR`** — closes the window, but monotonicity then depends on Redis
>   counter DURABILITY. A restart that loses the counter reissues tokens already in
>   use, and the equality guard cannot tell a reissued token from a real one.
>   Silent, and it corrupts ownership.
> - **PostgreSQL lease** (chosen) — monotonicity stays where it is already durable,
>   beside the sequence and the fence column. Costs one round trip per acquisition,
>   on a path that already makes several.
>
> Fail-closed beats fail-silent. `users.subscription_lock_owner` and
> `subscription_lock_lease_expires_at` live on the row the fence already occupies,
> so taking the lease and allocating the fence are ONE guarded UPDATE with no
> interval between them. Redis stays the cheap contention gate; correctness moved
> to the database.
>
> **A consequence worth stating, because it changes existing behaviour.** A
> Redis-only fault — an evicted key, a failover that lost data — no longer hands
> ownership to a newcomer: the incumbent still holds the DB lease, so the newcomer
> gets a retryable 503 and re-runs. Previously it took over and fenced out a live,
> working holder. The harness's `loseLease()` was Redis-only and therefore stopped
...
>
> - The **rider row lock** belongs to the sequence `rider lock → OTID lock →
claimForStore → insert → commit`. There is no `claimForStore` to wrap: it
>   arrives in step 5. Adding a lock in 4.75 would mean adding it to the Stripe
>   path alone, which is exactly the asymmetry step 5 exists to remove — and the
>   Stripe path _cannot_ hold a rider row lock across its transaction anyway,
>   because it makes a Stripe round trip inside it and would pin a pooled
>   connection across external I/O.
> - The **transaction-safe dedup** hazard (a `23505` on the reconciliation insert
>   aborting the surrounding transaction with `25P02`) is **latent today, not
>   live**: `runExclusive` hands the flow the _pool_ manager, not a transaction,
>   so a duplicate-key error there aborts nothing. It becomes reachable only when
>   step 5 puts the insert inside the claim transaction. Fixing it now would be
>   guarding a statement against a transaction that does not yet enclose it.
>
> This is the third time this mechanism has been mis-scoped by assuming an item
> is buildable because it is _describable_. The test is not whether the sentence
> makes sense in 4.75 — it is whether the code it names exists there.
>
...
WHERE id = $1 FOR UPDATE`, or `pg_advisory_xact_lock(rider)` — the harness
   ...
   >
   > Step 4.75 therefore carries, in the same change (**both ✅ shipped** —
   > migration 1834 and the entity union):
   >
   ...
   > without retirement. Tracked with step 4.75; it has the same
   > touches-live-code character.
   >
   > **✅ Change 1 shipped (2026-08-08) for the Stripe claim.** Both successful-claim
   > paths retire: the transition-UPDATE win (already durable, so retirement runs on
   > the pool manager) and `claimForStripe` (retirement inside the same transaction).
   > `claimForStore` has no equivalent because it does not exist yet — that lands in
   > step 5. **Change 2, the drain-side re-query, is still open**, so a Stripe
   > terminal clear that empties the slot still leaves conflict rows open.
   >
   > **Retirement is scoped by rider AND subscription id, and the rider half is
   > the non-obvious one.** A conflict row is filed against the rider whose event
   > LOST, so an open row carrying subscription `S` may belong to a DIFFERENT
   > rider — it records "rider B's event referenced `S`, which rider A owns".
   > Rider A reasserting ownership of `S` does NOT make that row moot: B is still
   > billed for a subscription bound to someone else, which is precisely what the
   > operator has to see. This is INV-C's shape again — supersession is only
   > available where the later writer may overwrite the earlier one, and across
   > riders ownership is exclusive, so it is not. A subscription-id-only filter
   > silently deletes the record of a real cross-rider anomaly; it was written
   ...
   >
   > **Where it actually landed (2026-08-08):** inside the claim transaction, with
   > **no** rider lock — that lock moved to step 5 with the rest of the store claim
   > path. On the Stripe path the transaction is sufficient by itself, because the
   > claim's own `WHERE` guard is what decides ownership; the lock was only ever
   > needed to serialise a _store_ claim against a concurrent one.
...
> **✅ ANSWERED 2026-08-12 by RevenueCat support (ticket `ZERPGP-3L1PN`) — and it
> is the bad answer. See step 4.5's canonical record in §12.** _"No. Google treats
> the replacement as a new purchase and creates a new purchase token and order
> root. This means RevenueCat will report a new `original_transaction_id` for the
> replacement subscription."_
>
> So this is a **confirmed defect**, not the possible non-issue it was written as:
> a rider upgrading monthly → annual hits `'conflict'`, gets a reconciliation row,
> keeps being billed on the plan they just upgraded to, and — because the claim is
> rejected wholesale — their `subscription_current_period_end`, status and tier
> stay frozen at the superseded subscription's values.
>
> **What the answer does NOT settle is the escape path, and the residual is
> sharper than the original question.** This item requires the path accept **only
> a store-confirmed supersession**, never merely a different id. Support names
> Google's linkage mechanism — the replacement purchase carries a
> **`linkedPurchaseToken`** pointing at the token it replaces — but §1's first
> correction already establishes that **RevenueCat exposes no Play purchase
> token**, and therefore no `linkedPurchaseToken` either. **The one signal that
> distinguishes "the store superseded A with B" from "this rider bought a second,
> independent subscription B" is not reachable through RevenueCat's data.** That
> is precisely the discrimination this item forbids guessing at.
>
> **Support's own recommendation sidesteps it rather than solving it, and it lands
> on a different part of this design than it first appears:** _"I wouldn't use
> `original_transaction_id` as the permanent link to a rider. I'd use an
> identified RevenueCat App User ID and allow the same rider to have more than one
> store transaction chain."_ The first half is **already how this design works** —
> §2 resolves riders by `app_user_id` against `users.purchase_account_token`, and
> the store identifier has never been the rider link. The second half is the load
> bearing one, and it is an instruction about **exclusivity**, not resolution:
> one nullable column per provider cannot represent a rider holding more than one
> chain, so "allow more than one" is a **storage-shape change**, not a guard tweak.
>
> **✅ FORK DECIDED 2026-08-12 — multi-chain storage.** Store chains move to their
> own table and entitlement derives from the live set, so an upgrade inserts chain
> B alongside chain A and **nothing has to decide whether B may replace A** — the
> store-confirmed-supersession signal RevenueCat cannot provide stops being
> needed. Design:
> `docs/superpowers/specs/2026-08-12-store-subscription-chains-design.md`.
>
> The reasoning that settled it: the equality guard's job was to stop a rider
> holding two subscriptions, and **it cannot do that** — it only stops us
> _recording_ the second one, while Google bills the rider either way. The rider
> then pays twice and sits on `free` because the claim returned `'conflict'`.
> Recording both and opening a reconciliation row is strictly better on every
> axis. The rule inverts: **detect and reconcile, do not refuse to persist.**
>
> This also settles **(h)** and the storage half of **(b)** — all three were the
> same single-slot assumption. What survives in (b) is the _re-query_ correlation
> against RevenueCat's subscriber API, which the 4.5 spike still has to answer.

_Deliberately NOT pre-emptively fixed._ Adding a replacement escape path to the
identity guard **still** must not be done by reflex, even now the behaviour is
confirmed: the guard's job is to stop a second, independent subscription taking a
slot that is already owned, and — per the answer above — no RevenueCat-visible
field separates the two cases. An "accept any different id on Google" bypass would
...
>   always): there is no success to wait for, so erasure is not gated **on a
>   cancellation** — **updated 2026-08-12: it IS gated on unresolved export
>   enrichment**, because erasing clears the only handle that can match a
>   lost-webhook Apple row to the export. The
...
| 4.5  | **⚠️ NARROWED 2026-08-12 — three of the four outputs are answered by RevenueCat support (ticket `ZERPGP-3L1PN`); see the canonical record above the spike narrative below. ONLY (b)'s correlation remains open — (f)'s fork is CLOSED (multi-chain storage, step 4.9), and do NOT treat the chain move as deferrable. **CORRECTED 2026-08-12 (later the same day): provision BOTH setups.** An earlier revision of this row said not to provision the two-base-plans replacement experiment, on the grounds that support had answered (f). It answered the identifier question; it did not answer whether the two chains of a replacement are separable on **re-query**. The re-query-both rule that now underpins every escalation needs each member correlated individually, and if both chains map to one product-keyed subscriber entry the worker cannot tell which stopped billing and can promote a legitimate replacement into the refund path. So the spike needs the two-independent-products setup for (b)'s correlation AND the replacement setup for its response shape. Still required, but ONLY for (b)'s correlation, which was never asked — so provision the two-independent-products setup — and, per the correction below, the two-base-plans setup as well, for a different question than the one support answered. (f) is answered — a confirmed defect whose fork is now CLOSED in favour of multi-chain storage (step 4.9), 6.5's Google cancellation is answered positive (RC v1 `.../cancel`), and the replay source is answered positive on BOTH fields (Scheduled Data Export transactions feed) — so the purchase-intent ledger fallback is NOT forced.** Original text: **Provisioning spike — four required outputs: (f) identity across a plan upgrade, **(b)** correlation with two simultaneous subscriptions, step 6.5's Google cancellation mechanism, and **whether RevenueCat exposes a replay source supplying BOTH rider discovery AND `original_transaction_id`** — a subscriber listing satisfies "a listing exists" while omitting the identity, which discovers the billed rider and still cannot call `claimForStore`; **either field missing is a NEGATIVE answer**, which forces the server-side fallback (a purchase-intent ledger written before the store call) — client-supplied repair is an accelerant and does not satisfy this. **The ledger is a DEGRADED substitute, not an equal one:** it supplies discovery but cannot supply `original_transaction_id`, so the repair grants entitlement with a **NULL identity binding** that the next renewal binds. Taking that route therefore also requires **(h)'s NULL-binding terminal strategy and its coverage** — a terminal arriving before the rebind matches nothing — so a negative answer here creates work in (h) as well, and must not be read as the question being closed (§4: without a discovery-and-identity source, scheduled reconciliation cannot find or claim a rider whose first webhook was lost — a negative answer forces choosing another mechanism **before** step 5, it does not defer).** RevenueCat project and a throwaway internal-testing build, with **two setups**: two base plans of one subscription for (f)'s replacement flow, and **two independent subscription products** for (b) — a second base plan replaces the first, so it can never produce the two simultaneously live subscriptions (b) needs — the subscriber response is keyed by product id and carries no original transaction identifier, so there is nowhere else to check it cheaply. **Record all four before teardown.** Skip only if RevenueCat support answers **all four** first — and for this one, only if the answer covers **both fields**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **spike still required, for (b) only** — narrowed 2026-08-12                                                                                   |
| 4.75 | **Fence ownership + conflict retirement, harness-first — store-free half only.** Build the real-Postgres + real-Redis concurrency harness (both orderings, plus a forced stall between `acquire()` and the stamp), then land the fix it validates. The harness selected acquisition-stamping plus a **DB-backed lease** (migration 1835), which makes "token order = acquisition order" true by construction rather than dropping it — one guarded UPDATE takes the lease and allocates the fence, so the equality-guard candidate was never needed; `publishFence()` **is deleted** — the harness selected acquisition-stamping, and the lease now exposes read-only `assertFenceCurrent()` in its place (shipped). Plus the `sbr_resolution_check` migration + entity union (shipped) and retirement-on-successful-claim **for `claimForStripe`**, which exists today (shipped). **The rider row lock and the transaction-safe reconciliation dedup moved to step 5 (corrected 2026-08-08)** — both name the store claim path, and `claimForStore` does not exist here; see the scoping table in §4. Touches live merged code. Runs harness cases (i)/(ii-a)/(v) only — the five needing a store writer run in step 5, which is not complete until they pass. **Blocks step 5.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | ✅ **done (2026-08-08)** — fence half, migration 1834, retirement, and the acquire-to-stamp gap closed by the DB-backed lease (migration 1835) |
| 4.9  | **Store subscription chains — the multi-chain storage move. Added 2026-08-12; BLOCKS step 5.** Design: `docs/superpowers/specs/2026-08-12-store-subscription-chains-design.md`, which is authoritative on the rules; this row is authoritative on the **scope**, enumerated so nothing falls out of it. **Ships as TWO releases.** **RELEASE A — (1) Expand migration:** `store_subscriptions` + indexes (vacuous backfill); the rollup columns `users.store_subscription_tier` + `store_subscription_tier_expires_at` with their paired CHECK; the `store_deletion_obligations` table (including `escalated_at`, without which a permanent failure files an operator incident on every sweep tick); the overlap-pair columns on `store_billing_reconciliations`; the widened `sbr_reason_check` (+ `provisional_overlap`, `ownership_conflict`) and `sbr_status_check` (+ `provisional`, `retired`) — **enumerated from the LIVE schema**, which already includes `unrecognized_product`; re-scoping `uq_sbr_open_apple_otid_reason` (mig 1823), which pairwise overlaps break with `23505` on the escalation path; and the partial sweep indexes. **(2) Writers:** store claims stop dual-writing the legacy Stripe-owned `users.subscription_*` — but **still maintain both rollup columns atomically with the chain row**. "Chain-only" is the wrong reading and leaves every store-only payer resolving as `free`. **(3) Readers — all of them:** `resolveEntitledTier` takes the rollup **and its expiry** (13 call sites); `/users/me` serves the billed tier across all sources, or the mobile activation poll never observes a purchase; **admin** projections **and** filters (`toRow`, `getById`, the flag list) via a single lateral join, no per-row fan-out; the **GDPR export**, which walks a fixed repository list and would otherwise omit store subscriptions entirely; and **`AccountService.buildSubscriptionSnapshot` with the representative election** — the design's whole projection section. Omitting it leaves the subscription page reading the now Stripe-owned columns, so a store-only payer is shown `free` / `canceled` with no provider and no management target **while `/users/me` and every feature guard already recognise the purchase** — the two halves of the product disagreeing about whether they paid. **(4) The overlap machinery itself, not just its schema** — creating provisional pairs on a second future-billing source, retiring them when either member stops future billing, and the bounded sweeps behind it — all of which are **local** and need no RevenueCat client. **Plus the expired-rollup recomputation worker**, which is a separate obligation and is not implied by creating its index: the synchronous resolver deliberately ignores the **whole** store rollup once it expires, so a rider whose Premium chain lapses while a Pro chain is still live falls to their Stripe/grant tier — and **stays there permanently** without the worker, rather than until the next tick. The accepted under-grant is only acceptable because it is bounded; shipping the index without the recomputation makes it unbounded. **The re-query-confirmed PROMOTION lands with step 5, not here**: confirming that both members are still billing requires the RevenueCat client, which does not exist until step 5, and step 5 is blocked on this step — so requiring it in release A is a deadlock. Provisional rows accumulate safely in the meantime: they are deliberately **not** operator items, and the promotion path is what turns them into one. Release A must ship the detection, not merely the columns; step 5 completes the escalation. The columns and indexes without the behaviour ship a system that **never detects double billing at all**, which is the problem this whole design exists for. **(5) Stripe checkout eligibility becomes chain-aware** — without it `createCheckoutSession` sees an empty Stripe side for a store-only rider and **sells them a second subscription**, the one place this design would create the double-billing it exists to detect. **(6) Notification delivery becomes source-aware** — the job payload carries provider + source identity and `stillMatches` validates that source, or a store purchase confirmation is discarded as superseded. **(7) Companion:** the Stripe portal must be reachable whenever a Stripe side exists (today `isStoreManaged` suppresses it), and `payment_method` / `billing_history` must be labelled as Stripe under a store representative. **(8) The Stripe snapshot carries the selected subscription id AND its `created`** — both are needed for pair identity and the refund-target chronology. **NOT in this step:** the RevenueCat-backed deletion enumeration and cancellation, which need a client that does not exist until step 5 — 4.9 carries only their **schema**; the mechanism lands with **6.5**. **RELEASE B — contract:** drop `apple_original_transaction_id` / `google_original_transaction_id` and their unique indexes, **gated on release A being fully deployed and no longer a rollback target** (migration 1831's `42703` rolling-deploy hazard; the vacuous backfill does not neutralise it, since the hazard is the column name in TypeORM's select list. Migration 1833's never-deployed shortcut is a statement about deployment state — re-check it, do not inherit it). **This must precede the consumer:** building step 5 against the retired single-slot guard produces a claim path that rejects a legitimate Play upgrade. | **not started — blocks step 5**                                                                                                                |
| 5    | **DEPENDS ON STEP 4.9 (added 2026-08-12) — the chain table, readers and projection must exist first; do not build this against the retired single-slot binding.** Backend: RevenueCat webhook consumer + **scheduled drift reconciliation** (an audit requirement the design had omitted — the inbox cannot recover an event that never arrived) + contract artifacts. **Also carries, moved from 4.75 on 2026-08-08:** the unconditional rider row lock in the claim transaction (`rider lock → OTID lock → claimForStore → insert → commit`) and the transaction-safe reconciliation dedup — a `23505` on the insert must not abort the enclosing transaction with `25P02`. Both are unbuildable before `claimForStore` exists, and the dedup hazard is not even reachable until the insert moves inside a transaction                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | buildable — see the blocking note below                                                                                                        |
| 5.5  | **Scheduled Data Export ingestion — the lost-first-purchase safety net. Added 2026-08-12; blocks Play/App Store ENABLEMENT.** Configure the RevenueCat Scheduled Data Export destination, ingest its transaction batches, keep a durable **checkpoint** so batches are processed exactly once and can be replayed, and reconcile `original_store_transaction_id` + the RevenueCat user identifier against our chains — claiming **every exported transaction absent from `store_subscriptions`, whatever the rider's current tier**. NOT just the billed-with-no-entitlement cohort: a lost webhook for a rider's SECOND subscription leaves them fully entitled from Stripe or another chain, so a tier-based filter skips it, the chain is never persisted, no provisional overlap is created, and the duplicate charge runs indefinitely — the exact case the overlap machinery exists for, made invisible to it. **This is what discharges the purchase-intent ledger** (see step 4.5's canonical record): a rider whose FIRST webhook was lost is still `free` with no binding, so no drift cohort of ours can discover them and they stay billed with no entitlement indefinitely. Cut this step and the ledger returns.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **not started — blocks enablement**                                                                                                            |
| 6    | Mobile: SDK, binding, paywall, preflight, purchase, poll-until-reflected, restore, one call site                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | not started                                                                                                                                    |
| 6.5  | **Account-deletion store cancellation — blocks production enablement.** Google: **✅ MECHANISM RESOLVED 2026-08-12 (support, `ZERPGP-3L1PN`) — server-side stop-renewal EXISTS.** `POST /v1/subscribers/{app_user_id}/subscriptions/{store_transaction_identifier}/cancel` (API **v1**, v1 secret key) stops the Play renewal without refunding or ending access early, so Google is a **durable server-side cancellation** and is NOT rider-driven; the "otherwise" branch is dead. **Two consequences.** (1) The erasure rule above therefore BITES for Google: erasure waits for cancellation **success**, and `purchase_account_token` is retained until both finish. (2) **STORAGE GAP — the path parameter is the CURRENT Google order identifier, not the original.** Migration 1833 dropped `google_store_transaction_id` and we persist only `google_original_transaction_id`, so the required parameter is not in our schema. Resolve by **fetching it at deletion time** from `GET /v1/subscribers/{app_user_id}` (whose per-subscription `store_transaction_id` is the current period's transaction — §1's first correction) rather than re-adding a column: no migration, no fourth generation of the identifier churn §6 cites as a real carrying cost, and the retain-until-cancelled rule already guarantees the token is still present when the call is made. **⚠️ SINGLE-CHAIN ASSUMPTION SUPERSEDED 2026-08-12 by step 4.9:** this row still says fetch _the_ current order identifier and cancel _the_ subscription. Under multi-chain storage a rider can hold several live Google chains, so deletion must **enumerate all of them and cancel each**, and erasure waits for **every** cancellation to succeed rather than the first. The retry state cannot live in `store_subscriptions` — that table cascades on user delete, so the purge removes the very rows listing what still needs cancelling; it belongs in this row's purge-safe non-cascading table, **one entry per still-live chain**, not one per rider. See the deletion section of `docs/superpowers/specs/2026-08-12-store-subscription-chains-design.md`. Apple: no server-side cancel exists, so this is rider-action product copy plus **purge-safe** state for support. **⚠️ UPDATED 2026-08-12 — Apple erasure is NOT simply ungated any more.** There is still no cancellation to wait for, but a no-ID Apple support row must be **enriched from the Scheduled Data Export while `app_user_id` still exists**, so erasure waits on that enrichment, bounded by a persisted `enrichment_deadline_at`. Erasing at purge without it clears the only handle and forces the row into the unmatchable path, where it can be swept while Apple is still billing. See the chain design's deletion section — `store_billing_reconciliations` cascades on user delete, so the record must live in its own non-cascading, minimised, retention-bounded table. Also adds **RevenueCat subscriber erasure** to finalisation (the purge deletes the Stripe customer and has no RC counterpart), with a durable retry that outlives the purged row. **Where server-side cancellation exists, erasure waits until NO ACTIONABLE cancellation remains — no row `pending` or `failed` in the current attempt (updated 2026-08-12; NOT "every row succeeded", which an Apple `support_only` row can never satisfy, so a rider holding both an Apple and a Google subscription would block forever) — not merely an attempt — and the work item retains `purchase_account_token` until both finish, then drops it.** Erasing after a transient stop-renewal failure destroys the handle the retry needs and leaves the deleted rider renewing indefinitely. Where no server-side cancel is possible (rider-driven, and Apple always), erasure is not gated. The local purge is never blocked by either. Today `requestDeletion` (`account-deletion.service.ts:183-216`) and `purgeStillDueUser` (`:770`) handle Stripe only, so a store subscriber who deletes is locked out and keeps being charged. **⚠️ AND THE FINAL 'ungated' CLAUSE IS QUALIFIED (2026-08-12): Apple erasure waits for no CANCELLATION, but it DOES wait for unresolved export enrichment** — erasing clears `app_user_id`, the only key that can match a lost-webhook Apple row to the Scheduled Data Export, so erasing first destroys the handle and forces the row into the unmatchable path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **not started — newly required 2026-08-07**                                                                                                    |
...
| (f)  | **✅ SUPERSEDED 2026-08-12 — do NOT build the equality-only guard.** This row previously read "gates ENABLING Play purchases; build with the equality-only guard; Apple-only enablement is not gated at all", and all three clauses are now wrong: the fork is decided (multi-chain storage), the single-slot binding is retired, and mobile IAP ships both stores together so there is no Apple-only enablement to be ungated. The consumer is built against the chain table — see step **4.9**, which must land before step 5. Support confirmed a Play replacement presents a new `original_transaction_id`, which removed the possibility that the equality guard was already correct; the fork that answer opened is **closed**, not outstanding. See (f) for the decision.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| (b)  | **⚠️ STORAGE HALF RESOLVED 2026-08-12 by the multi-chain decision (see (f) and `docs/superpowers/specs/2026-08-12-store-subscription-chains-design.md`); the RE-QUERY half still blocks COMPLETING step 5.** Correlating an event to a rider's chain is now a primary-key lookup on `(provider, original_transaction_id)` against our own table, so a failed correlation degrades a re-query instead of corrupting a binding. What is NOT resolved is verifying a chain against RevenueCat's subscriber API, which is still keyed by product id with no original transaction identifier — the new table carries `product_id` for exactly that, and the 4.5 spike is still what proves it works. Original text: **Blocks COMPLETING step 5, not starting it.** The identity field is settled; the **correlation** is not. The subscriber API keys subscriptions by product id and does not return an original transaction identifier, so with two Play subscriptions the consumer cannot verify which entry the event's `original_transaction_id` names. Applying the wrong entry writes one subscription's active-or-terminal state under the other's identity. Likely rule — match on the event's `product_id`, which keys the subscriber response — but **verify it in the 4.5 spike rather than assuming**, and land the two-subscription regression test in the same PR as the claim. |
| (g)  | **⚠️ RESTATED 2026-08-12 for the multi-chain model — the risk survives, its mechanism does not.** This row described a poisonable **NULL identity slot** under an equality-only claim guard. Step 4.9 retires both, so the acceptance criterion as written is unsatisfiable and chasing it would rebuild the single-slot guard. The underlying risk is unchanged: an event the backend cannot verify now manifests as **insertion of a bogus entitling chain row**, which grants a tier nobody paid for and creates overlaps against the rider's real subscription. (g) therefore requires **validation before chain insertion** — an event must be supported by the authoritative subscriber re-query before a chain is written — with coverage that an event NOT supported by that re-query inserts nothing. Original text: **Blocks COMPLETING step 5, not starting it.** The NULL-identity branch accepts any event-supplied identifier and the re-query cannot verify it, so shipping the equality-only claim without a response leaves a poisonable first binding — and a poisoned one rejects the rider's own later expiry/refund, leaving entitlement active. Choose a response from (g) and land its regression coverage **in the same PR as the claim**, not after.                                                                                                             |
| (h)  | **✅ RESOLVED 2026-08-12 by the multi-chain decision — structurally, not case-by-case.** A terminal marks one chain `canceled`; the other rows are untouched and entitlement recomputes from what remains live, so "recompute from the full set rather than apply the event's terminality" becomes the only thing the code CAN do. See `docs/superpowers/specs/2026-08-12-store-subscription-chains-design.md`. Original text: **Blocks COMPLETING step 5.** A terminal event for one of a rider's two store subscriptions would clear the single identity slot and drop the tier to `free` while the other keeps billing. The terminal path must recompute entitlement from the full re-queried set rather than apply the event's terminality, switch the binding atomically (the `IS NULL OR = :otid` guards block a direct A→B move), and pick a deterministic winner when several remain entitling.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
> **⚠️ THREE OF THE FOUR OUTPUTS ARE ANSWERED — RevenueCat support, 2026-08-12
> (ticket `ZERPGP-3L1PN`, Erik, Developer Support Engineer). This is the
> canonical record; (f), (b), 6.5 and the replay question cross-reference here.**
>
...
> token and order root. This means RevenueCat will report a new
> `original_transaction_id` for the replacement subscription."_ The equality-only
> guard is therefore **not** already correct, and (f) is a confirmed defect rather
> than the possible non-issue it was written as. **But this does not settle the
> escape path**, and the residual is sharper than the original question — see (f).
>
> **2. Step 6.5's Google cancellation mechanism — ANSWERED POSITIVE.**
> `POST /v1/subscribers/{app_user_id}/subscriptions/{store_transaction_identifier}/cancel`
> (API **v1**, v1 secret key) stops a Play subscription renewing without refunding
> or ending access early. **Google only — support confirms RevenueCat exposes no
> Apple cancellation endpoint**, so Apple stays rider-driven exactly as 6.5
> assumed. Note the path parameter is the **current Google order identifier**, not
> the original — see 6.5 for what that costs us.
>
> **3. (b)'s multi-subscription correlation — NOT ASKED, still open.** It was not
> among the three questions put to support. This is the whole remaining reason the
> spike exists.
>
> **4. The replay source — ANSWERED POSITIVE on BOTH required fields.** There is
> no project-wide events API (_"The API v2 events endpoint only works when you
> already know the customer"_), which confirms this document's reading. But the
> **Scheduled Data Export transactions feed** _"gives you a project-wide list of
> new and updated transactions. It includes RevenueCat user identifiers and
> `original_store_transaction_id`"_ — discovery **and** identity from one source,
> which is exactly the both-fields test this spike was told to apply. **The
> purchase-intent ledger fallback is therefore NOT forced**, and neither is the
> NULL-binding terminal work in (h) that a negative answer would have created.
> Caveat to carry into step 5: it is a **scheduled batch drop to cloud storage**,
> not an API and not a replay — hours of latency, fit for a reconciliation sweep,
> useless for anything synchronous.
>
> **⚠️ AND IT IS ONLY AN ANSWER IF WE ACTUALLY BUILD IT (added 2026-08-12).** This
> answer is what discharges the purchase-intent ledger, so the discharge is only
> valid once the export is **delivered infrastructure**, not a capability that
> exists at the vendor. The ordinary drift cohorts cannot help here: a rider whose
> **first** webhook was lost is still `free` with no store binding, so nothing in
> our own data distinguishes them from a rider who never purchased — they are
> billed with no entitlement, indefinitely, and only a project-wide source finds
> them. **Step 5.5** below therefore carries the export destination, the batch
> ingestion, the checkpoint/replay semantics and a lost-first-purchase test. If
> that step is cut, the ledger comes back — they are alternatives, not
> independent.
