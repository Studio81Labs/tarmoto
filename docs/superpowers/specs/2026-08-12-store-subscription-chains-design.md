...
| `attempt_outcome`         | varchar(16)            | **New.** `running` \| `purged` \| `abandoned`, stamped on all of an attempt's rows when it ends. What the retention sweep reads — `attempt_id` alone is opaque and the deletion log carries no attempt reference                                                                                                                                                                                                                                                      |
...
| `target_key`              | varchar(1024) **NULL** | **New.** The target's identity, and a STAGED key: `original_transaction_id` once known, otherwise the observed `store_transaction_id` as a **provisional** value flagged by `target_key_provisional`. Enrichment **re-keys it to the original and merges** any row resolving to the same one — it is not immutable, and treating it as first-observed-forever keeps a pre-renewal and a post-renewal observation of one chain as two targets. NULL for `erasure` rows |
...
**merged into it**: one row survives and the loser is `retired`. **`succeeded` wins the
merge** — explicitly, not "the most actionable status", which would elect a `pending` or
`failed` duplicate over a row that has **already stopped renewal**, leaving the merged row
gating erasure and retrying an external operation that is complete. Precedence is
`succeeded` > `support_only` > `failed` > `pending`: a proven outcome beats an unfinished
attempt at the same chain. Two rows for one chain therefore converge instead of one blocking erasure after the
other succeeds.
...
**deleted** rider. **No product-scoped replacement rule.** An earlier draft retired an existing non-actionable
row when a different same-product chain appeared; with `target_key` distinguishing chains that
would **delete a live obligation** — two Apple chains of one product leave the first
`support_only`, and retiring it strands a still-billing subscription with no record once the
chain rows cascade at purge. Distinct targets simply get distinct rows.
...
`kind = 'cancellation'` **requires** `provider`, `product_id`, `store_transaction_id` and
`target_key`; `kind = 'erasure'` requires **none** of them — including `target_key`, which
names a chain an erasure row does not have, and whose uniqueness is `(attempt_id)` alone. The
CHECK is what keeps the loosened column types from admitting a cancellation row with no
target. Also **not** `original_transaction_id`, which the enumeration often cannot supply —
...
while ACTIONABLE (`pending` / `failed`) for BOTH kinds, AND while a `support_only` row still
owes enrichment** (`export_matchable` true and no `original_transaction_id` yet). A
lost-webhook Apple row is `support_only` **immediately** and has no stable id, so an
actionable-only requirement lets the database accept it **without the one handle that can
match the export** — and it is then forced down the degraded unmatchable path while the handle
was still available. Null is permitted only **after** enrichment resolves it, one way or the
other, not for erasure alone: every cancellation attempt and every
...
attempt's **outcome**, which means the outcome must be **persisted**: `attempt_id` alone is an
opaque UUID with nothing to resolve it against, and the existing deletion log records events
with no attempt reference, so repeated attempts cannot be mapped back. Every obligation
carries **`attempt_outcome`** — `running` | `purged` | `abandoned` — stamped on all of the
attempt's rows when the purge completes or a restore abandons it. Without it the worker must
either retain abandoned-attempt identifiers indefinitely or risk deleting evidence for an
attempt still in its grace period.
...
free and denied paid features they are being charged for**. Restoration inserts the chain from the
re-query and recomputes the rollup rather than reconciling nothing — **which requires
...
- **Merge precedence — a `succeeded` duplicate beats a `failed` one.** The survivor must not
  keep gating erasure or retrying an already-completed cancellation.
- **Retention — the sweep reads a persisted `attempt_outcome`**, not the absence of a purge;
  an abandoned attempt is distinguishable from one still in its grace period.
