...
| `road_quality_max_zoom`  | `12` | `null` (∞)                 | Road-quality overlay zoom clamp — **client-side on both platforms** (the shared `clampQualityMaxZoom` fed to the MapLibre layer `maxzoom`): **companion** `/explore` + planner (`resolveQualityLayerMaxZoom` / `MapCanvas`, `/config/limits` map for anonymous viewers) and **mobile** `MapScreen` overlay (`MOBILE_QUALITY_CEILING` = `22`, the mobile tile source's real max — companion's is `18`). No backend endpoint enforces it |
