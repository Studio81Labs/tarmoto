...
**both** companion and mobile) with a server-side tile backstop; its
anonymous leg is still gated on `TARMOTO_TILES_ANON_QUALITY_ZOOM_CLAMP_ENABLED`
but is no longer blocked on client work — see the table row above and the
ordering note under [Storage](#storage).
...
introducing tier gating changed nothing for current users. The enforced limits
shipped dark the same way — a launch-mode global row (`limit_states`, value
`NULL` = unlimited) that suppressed the free-tier cap.

**Those launch-mode overrides are gone.** Migration `1839` (#1287) deleted all
nine of them at monetization go-live, so `max_active_trips` (`free = 1`),
`max_trip_collaborators` (`free = 0`) and `road_quality_max_zoom`
(`free = 12`) now resolve for real. An operator can still add a global
override by hand (`PUT /admin/feature-limits/:feature/global`), but nothing is
seeded any more — a fresh database resolves the registry tiers directly.

### Ordering constraint: the anonymous tile clamp

`TARMOTO_TILES_ANON_QUALITY_ZOOM_CLAMP_ENABLED` is the last unflipped switch
on `road_quality_max_zoom`, and it had exactly one precondition: tile fetches
had to carry identity first. Both halves of that are now true:

1. the 1839 seed-clearing above landed, so the free cap resolves to `12`;
2. #1279 gave every live tile consumer a credential — the companion and mobile
   MapLibre sources append a scoped `tile_token`, and the mobile offline-pack
   downloader sends the rider's bearer.

With both in place the flip is safe: it clamps only genuinely anonymous
requesters (signed-out visitors, scrapers, and app versions predating #1279),
which is the free-tier view they are entitled to. Flipping it BEFORE #1279
shipped would have severed pro/premium deep zoom on both clients and every
offline pack, which is why it shipped dark.

**To flip:** set `TARMOTO_TILES_ANON_QUALITY_ZOOM_CLAMP_ENABLED=true` on the
backend service in Coolify and redeploy. It is read per request, so no code
change and no migration is involved, and setting it back to `false` reverts
the behaviour on the next request.
