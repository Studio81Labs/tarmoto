...
| `account`           | Subscription billing snapshot, Stripe checkout / portal, webhooks |
...
Background work runs on **BullMQ** (Redis-backed). The `jobs` module owns the connection, registers fifteen named queues (`ALL_QUEUE_NAMES`), schedules recurring jobs as BullMQ repeatables on application bootstrap, and exposes operational counters at `GET /jobs/health`.
...
| `osm.import`                | weekly Sun `0 1 * * 0`            | Imports the configured OSM `.osm` extract into `road_segments` (#781), ahead of POI import + fun-zones. Dormant until `TARMOTO_OSM_IMPORT_ENABLED=true`.                                                                                                                                                                                                                                                                                            |
| `poi.import`                | weekly Sun `0 3 * * 0`            | Dispatcher: fans out staggered `import-region` children that mirror OSM/FSQ POIs into `pois` (#745, #850). Dormant until `TARMOTO_POI_IMPORT_ENABLED=true`. **Not scheduled or processed here** — the backend only registers this queue to enqueue admin-triggered manual imports; the recurring schedule and the worker both run in the separate `apps/ingest` deployable (see "Deploying `apps/ingest`" in the [runbook](../process/runbook.md)). |
| `quality.conflation`        | one-shot (after `osm.import`)     | Success-continuation of `osm.import`: injects an OSM `smoothness` tag per way so GraphHopper can weight quality-aware routes (#779). Dormant until `TARMOTO_QUALITY_CONFLATION_ENABLED=true`.                                                                                                                                                                                                                                                       |
...
Workflows: [`backend-deploy.yml`](../../.github/workflows/backend-deploy.yml) (triggers the Coolify deploy API, tracks the deployment to completion, runs smoke test, surfaces manual rollback instructions on failure), [`companion-deploy.yml`](../../.github/workflows/companion-deploy.yml) (staging + production), [`admin-deploy.yml`](../../.github/workflows/admin-deploy.yml) (admin console), [`marketing-deploy.yml`](../../.github/workflows/marketing-deploy.yml) (marketing site), [`mobile-release.yml`](../../.github/workflows/mobile-release.yml) (TestFlight + Play Internal). Post-deploy verification is `scripts/smoke/smoke.sh`.
