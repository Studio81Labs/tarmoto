...
| `poi.import`                | weekly Sun `0 3 * * 0`            | Dispatcher: fans out staggered `import-region` children that mirror OSM POIs into `pois` (#745, #850). Dormant until `TARMOTO_POI_IMPORT_ENABLED=true`.                                       |
