...
| `road.import`               | weekly Sun `0 1 * * 0`                    | Imports every configured region's OSM extract into `road_segments` (#781), ahead of POI import + fun-zones — the **folder model** (Sub-project B): a per-region `<code>.osm` (produced by `apps/ingest`'s `refresh-road-extracts` script) instead of one hand-prepared file, looped by `OsmImportService.importAll()`. Dormant until `TARMOTO_OSM_ROAD_IMPORT_ENABLED=true`. See "Road-quality extract refresh" in the [runbook](../process/runbook.md#road-quality-extract-refresh-sub-project-b).                                                                                                                                                                                                                                                                                                 |
