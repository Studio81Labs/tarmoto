...
- **Companion** runs as a container from [`apps/companion/Dockerfile`](../../apps/companion/Dockerfile) (Next.js standalone output, `node apps/companion/server.js`) on the same self-hosted PaaS as the backend (#1240 — previously Cloudflare Workers via `@opennextjs/cloudflare`). Staging on push to `main`, production on tag `v*` (no PR previews — review companion changes locally).
