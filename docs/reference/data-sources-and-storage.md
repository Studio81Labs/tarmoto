...
**Store + import — landed (#745).** The `pois` PostGIS table now exists (geometry Point 4326, `source` + `external_id` unique, `kind`, name/website/phone, `last_imported_at`; GiST + kind indexes), with a weekly `PoiImportService` (BullMQ `poi.import`, Sunday 03:00) that queries the §7 tag sets via `PoiProvider.findPointsOfInterestInBbox` and upserts by `(source, external_id)` — idempotent, and a provider outage aborts before any write so existing rows survive. Bounded to `TARMOTO_POI_IMPORT_BBOX` (default CZ/Beskydy). **Dormant by default** (`TARMOTO_POI_IMPORT_ENABLED=false`).
