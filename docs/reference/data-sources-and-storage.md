...
| POIs (fuel, food, viewpoints, accommodation)                | **Reads: live via `overpass.provider`** · store: `pois` (PostGIS, #745) | Overpass (OSM) + Overture gap-fill          | PostGIS store + per-request live reads | Store: weekly import · reads: live | none | `pois` table + weekly import landed (#745, dormant by default). Reads stay live-Overpass until the read-path switch lands **with offline packs**. See §8.3 |
...
- **Don't hand-maintain the accommodation tag set here** — the live contract is `ACCOMMODATION_KINDS`, exported today from `apps/backend/src/modules/poi/dto/accommodation.dto.ts` (today: hotel, guest*house, motel, hostel, chalet, apartment, **camp_site**), which `OverpassPoiProvider` already builds its tourism regex from. The stored/offline importer (§8.3) must source the kind list from that constant, not a copied list, so offline POIs can't diverge from `/accommodations?kinds=…` (e.g. dropping campgrounds). *(Per the `@tarmoto/shared` domain-enum convention this list arguably belongs in shared; until it's moved, the backend DTO is the single source — import it from there, not from `@tarmoto/shared`.)\_
...
**Store + import — landed (#745).** The `pois` PostGIS table now exists (geometry Point 4326, `source` + `external_id` unique, `kind`, name/website/phone, `last_imported_at`; GiST + kind indexes), with a weekly `PoiImportService` (BullMQ `poi.import`, Sunday 03:00) that mirrors a region and upserts by `(source, external_id)` — idempotent, and a provider outage aborts before any write so existing rows survive. Bounded to `TARMOTO_POI_IMPORT_BBOX` (default CZ/Beskydy). **Dormant by default** (`TARMOTO_POI_IMPORT_ENABLED=false`). It imports the **full §7 storage set** — `PoiProvider.findImportPoisInBbox` covers food (incl. `fast_food`), fuel, viewpoints, rest areas (`highway=rest_area|services`), and ice cream (a **superset** of the live `PoiKind` enum, written to the free-form `pois.kind` so widening the store never changes the `/poi` API), plus `findAccommodationsInBbox` for the `ACCOMMODATION_KINDS` set.
