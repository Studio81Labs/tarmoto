...
| POIs (fuel, food, viewpoints, accommodation)                | **Reads: live via `overpass.provider`** · store: `pois` (PostGIS, #745) | Geofabrik/osmium (OSM) + Overture gap-fill  | PostGIS store + per-request live reads | Store: weekly bulk extract · reads: live | none | Store filled by the **Geofabrik → `osmium tags-filter` → per-region `.osm`** bulk import (#745, continent-scaled #850) — 17 countries, config-driven (`TARMOTO_POI_IMPORT_REGIONS`), **dormant by default**; Overpass is the live read-path fallback only. Stale-by-absence tombstone (`deactivated_at`, per-region-bbox-bounded). See §7 + §8.3 |
...
- **Coverage — 17 countries, config-driven.** CZ (launch) + SK, PL, DE, AT, IT, SI, HR, BA, RS, ME, MK, AL, XK, BG, RO, GR. Each region's authoritative bbox lives in `DEFAULT_REGIONS` (`packages/ingest/src/poi/regions.ts`) and is both the `osmium extract -b` clip and the tombstone boundary. Active set = `TARMOTO_POI_IMPORT_REGIONS` (comma-separated codes; unset = all 17); the import is **dormant by default** (`TARMOTO_POI_IMPORT_ENABLED=false`), extracts read from `TARMOTO_POI_IMPORT_DIR`.
...
**Write-path topology.** The extract, import, and POI-schema migrations all live in **`apps/ingest`** (`@tarmoto/ingest-service`) plus its two library packages — `@tarmoto/poi-db` (entities + migrations) and `@tarmoto/ingest` (pure OSM/FSQ mappers + the `poi.import` queue contract). The backend is a **POI reader plus a thin admin front-door**: it serves the store-first read paths, and its admin API only **enqueues** onto a producer-only `poi.import` queue (upload an extract, trigger a region import) — `apps/ingest`'s own BullMQ worker is what actually runs the weekly dispatch and any admin-triggered job. The shared `/data/poi-extracts` volume + Redis queue contract are unchanged by this split: the backend still uploads to the shared volume and `apps/ingest` still reads from it, and the two write paths never collide because an admin upload's `.<pid>.<hex>.part` temp suffix and the scheduled refresh's `.refresh.part` temp suffix stay distinct.

**Store + import — landed (#745), continent-scaled (#850).** The `pois` PostGIS table exists (geometry Point 4326; `source` + `external_id` unique; `kind`; name/website/phone; #848 decision-support columns; `last_imported_at`; `deactivated_at`; **GiST `geom` + GIN `tags` + `kind` + `(address_country, kind)`** indexes), with a weekly `PoiImportService` (now in `apps/ingest`; BullMQ `poi.import`, Sunday 03:00 UTC) that upserts by `(source, external_id)` — idempotent, and a provider/extract failure aborts before any write so existing rows survive. The import runs from **per-country Geofabrik `.osm` extracts** — `osmium tags-filter` to the §7 tag set → `osmium extract -b` to the region bbox → `<code>.osm` in `TARMOTO_POI_IMPORT_DIR` (see the [runbook](../process/runbook.md#producing-per-country-poi-extracts)) — **not** a live Overpass bbox, so it scales to the **17-region** coverage list (CZ + touring neighbours + the Balkans / SE-Europe corridor) without the Overpass public-API limits. Coverage is `TARMOTO_POI_IMPORT_REGIONS` (default all 17; each region clipped to its authoritative `DEFAULT_REGIONS` bbox). **Dormant by default** (`TARMOTO_POI_IMPORT_ENABLED=false`). It imports the **full §7 storage set** — `PoiProvider.findImportPoisInBbox` covers food (incl. `fast_food`), fuel, viewpoints, rest areas (`highway=rest_area|services`), and ice cream (a **superset** of the live `PoiKind` enum, written to the free-form `pois.kind` so widening the store never changes the `/poi` API), plus `findAccommodationsInBbox` for the `ACCOMMODATION_KINDS` set.
