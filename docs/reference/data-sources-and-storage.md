...
**Enablement view (Phase 3).** `GET /internal/poi/regions` reports rows for **enabled sources only** — a disabled source contributes zero rows rather than 17 `configured: false` ones — and each row's `configured` flag reflects whether `code` is in that source's own (possibly `TARMOTO_*_IMPORT_REGIONS`-narrowed) `regions` list. The manual trigger now **400s** a disabled or unconfigured `(source, code)` pair outright, replacing Phase 2's "advertise all 34 pairs, then skip gracefully" behavior; the worker's graceful skip (recording a `skipped` run instead of failing) still exists, but only as defense for a stale already-queued job, not the primary admin-trigger UX.

**New required config.** `TARMOTO_INTERNAL_API_TOKEN` — the same secret on **both** `apps/backend` and `apps/ingest` — and `TARMOTO_INGEST_INTERNAL_URL` (backend → `apps/ingest`'s internal address). Without them, the admin page's trigger/coverage/run-history calls fail (`503` — or `401` if the token is set on both sides but doesn't match); the local upload path (admin → backend → shared volume) still works, since it never calls the internal API. Deploy order and degradation detail: [runbook](../process/runbook.md).
